    ...
    submitGuess: async ({ word }: { word: string }) => {
      ...
      if (normalizedWord === wordToGuess) {
        ...
        if (roundNumber < MAX_ROUNDS) {
            ...
            return `The user's guess "${normalizedWord}" was CORRECT. They won the final round, scoring ${roundScore} points. This was the last round! Their final total score is ${newTotalScore}. Thanks for playin', and have yourself a mighty fine day!`;
        }
      } else {
        if (newGuessedWords.length >= MAX_GUESSES_PER_ROUND) {
            ...
            if (roundNumber < MAX_ROUNDS) {
                ...
                return `The user ran out of guesses on the final round. The game is over. The word was "${wordToGuess}". Their final score is ${totalScore}. Thanks for playin', and have yourself a mighty fine day!`;
            }
        } else {
